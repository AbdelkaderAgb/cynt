<?php
/**
 * Transfer Invoices View — Tailwind CSS
 */
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white"><?= __('transfer_invoice') ?: 'Transfer Invoices' ?></h1>
        <p class="text-sm text-gray-500 mt-1"><?= number_format($total) ?> records found</p>
    </div>
    <a href="<?= url('transfer-invoice/create') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 transition-all hover:-translate-y-0.5">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400"><i class="fas fa-inbox text-4xl mb-3 block"></i>No records found</td></tr>
                <?php else: ?>
                <?php foreach ($invoices as $inv): ?>
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                    <td class="px-4 py-3 font-mono font-semibold text-blue-600"><?= e($inv['invoice_no']) ?></td>
