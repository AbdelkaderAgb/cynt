<?php
/**
 * Tour Invoices List View — Tailwind CSS
 */
...
<div class="mb-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl" x-data="{s:true}" x-show="s" x-init="setTimeout(()=>s=false,3000)"><i class="fas fa-check-circle mr-1"></i>Tour invoice saved</div>
<?php endif; ?>
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white"><?= __('tour_invoice') ?: 'Tour Invoices' ?></h1>
        <p class="text-sm text-gray-500 mt-1"><?= number_format($total) ?> records found</p>
    </div>
    <a href="<?= url('tour-invoice/create') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-semibold shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all hover:-translate-y-0.5">
        <i class="fas fa-plus"></i> New Tour Invoice
    </a>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400"><i class="fas fa-file-invoice text-4xl mb-3 block"></i>No tour invoices found</td></tr>
                <?php else: ?>
                <?php foreach ($invoices as $inv): ?>
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                    <td class="px-4 py-3 font-mono font-semibold text-purple-600"><?= e($inv['invoice_no']) ?></td>
