...
class CreditNoteController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $countStmt = $db->prepare("SELECT COUNT(*) FROM credit_notes cn LEFT JOIN partners p ON cn.partner_id = p.id WHERE $where");
        ...
        $stmt = $db->prepare(
            "SELECT cn.*, p.company_name as partner_name, i.invoice_no 
             FROM credit_notes cn 
             LEFT JOIN partners p ON cn.partner_id = p.id
    ...
    public function store(): void
    {
        ...
        if ($id) {
            $stmt = $db->prepare(
                "UPDATE credit_notes SET invoice_id = ?, partner_id = ?, amount = ?, currency = ?, reason = ?, status = ? WHERE id = ?"
            );
            $stmt->execute([$invoiceId, $partnerId, $amount, $currency, $reason, $status, $id]);
        } else {
            ...
            $stmt = $db->prepare(
                "INSERT INTO credit_notes (credit_note_no, invoice_id, partner_id, amount, currency, reason, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            );
            $stmt->execute([$creditNoteNo, $invoiceId, $partnerId, $amount, $currency, $reason, $status, $userId]);
        }
